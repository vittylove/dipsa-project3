...
public class TopicDetails extends AppCompatActivity implements View.OnClickListener,FirestoreAdapter2.OnListItemClick{
    ...
    public void refreshRecycleView(){
        //Recylcer View
        Query query = firebaseFirestore.collection("Comment").whereEqualTo("topicId", topic.getTopicId());
        firebaseFirestore.collection("Comment").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                List<Comment> allComments = task.getResult().toObjects(Comment.class);
                comments = allComments.stream().filter(x -> x.getTopicId().equals(topic.getTopicId())).collect(Collectors.toList());
            }
        });
